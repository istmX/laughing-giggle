...
1=> Add loop in the context agnet to first it will rea dthe prompt analyse geenrate context and then check if it meets all teh requriment and prompt requirmet form the daat folder and prompt to and tne if not thne it will  fxi only those thinsg that doets match and then provide the context and keep the loop for a speic time liek max 3 time sand the give reuslt to prevnet token burn and user wait and till thne we will add osemthing intresting in ui so user dont get bore 




2=> sparating the  Ai playground from project menas adding one more conevrstion there fro user to talk with ai back annd fourth to genarte deisgn system they talk and until user is stifed and in the end tehy will crete a perfcet deisgn system  called DESIGN.md and can  pubish it to template  and oteh ruser can sue that formt will be something like this `## Overview
...
ai will use all teh daat we have for desin in bakcend isnde data folder the ui folder and format will be this and also genarte the deisgns yemt using html and taiwlind css so showcse all the thijg sin a privew using html embidings and privewr in our app the temlpaet section will renser the hatmla nd tehy can downl dteh md fiel or copy the desgin system  and user doenstnot have to to back and fourth with ai if tehy ahve a deisgn sytem tehy can pubish it anywase sut add the diesgn sysem in text source optionl and give it will go to ai so it cna make html and css deisgn system for disyply too 

3=> Usrnamae verifction and badges will be getting by postin gmore on tempaets and all since we ask for usernm whiele sihnup so we display user in the templete section and and get virided badge and also some loyalty badges  and all 


4=> admin dahbord since we are adding tese thigs we need to add admin panl admin page will ahev acces to all teh fesures in the app the whole app 




Extar fix the jdownld zip button at ui doenst work 