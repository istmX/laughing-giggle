...
- **PM Wizard Domain-Specific Interview Prompts**: Updated `conversational_prompt.py` and `question_prompt.py` with strict domain boundary rules. For portfolios, visual showcases, and landing pages, Zenix AI is strictly prohibited from asking database questions (PostgreSQL vs MongoDB vs Neo4j), server auth (Clerk), or websocket questions. Interview questions now focus exclusively on domain-relevant design choices (GSAP scroll motion vs subtle CSS transitions, category filter tabs vs continuous visual scroll, dark vs light aesthetics).

### Verification
- Python Py_compile (`app/prompts/conversational_prompt.py`, `app/prompts/question_prompt.py`, `app/langgraph/context_engine.py`, `app/langgraph/playground.py`) succeeded with 0 errors.
- Production Vite build passes cleanly in 19.67 seconds with 0 warnings or errors.
