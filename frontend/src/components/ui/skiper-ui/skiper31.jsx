...
const CharacterV2 = ({
  ...
  return (
    <motion.span
      className={cn("inline-block text-zinc-400 mx-4 font-semibold text-4xl lg:text-5xl tracking-tight transition-colors hover:text-zinc-950", isSpace && "w-4")}
      style={{
        x,
        scale,
        y,
...
const Skiper31 = () => {
  ...
  const text = "trusted by engineers";
  ...
  return (
      <section className="w-full bg-white relative z-20">
        <div
          className="top-32 absolute left-1/2 z-10 grid -translate-x-1/2 content-start justify-items-center gap-6 text-center text-zinc-950">
          <span
            className="relative max-w-[12ch] text-xs uppercase leading-tight opacity-40 after:absolute after:left-1/2 after:top-full after:h-16 after:w-px after:bg-gradient-to-b after:from-white after:to-zinc-950 after:content-['']">
            Scroll to explore
          </span>
        </div>
        <div
        ...
        <div
          ref={targetRef2}
          className="relative -mt-[100vh] box-border flex h-[210vh] flex-col items-center justify-center gap-[2vw] overflow-hidden bg-white p-[2vw]">
          <p
            className="flex items-center justify-center gap-3 text-2xl font-medium tracking-tight text-zinc-950">
            <Bracket className="h-12 text-zinc-950" />
            <span className="font-medium text-zinc-400">
              Integrate with your favorite tools
            </span>
            <Bracket className="h-12 scale-x-[-1] text-zinc-950" />
          </p>
          <div
            className="w-full max-w-6xl flex flex-wrap justify-center items-center gap-4 mt-8">
            {techStack.map((char, index) => (
              <CharacterV2
                key={index}
                char={char}
                index={index}
        ...
        <div
          ref={targetRef3}
          className="relative -mt-[95vh] box-border flex h-[210vh] flex-col items-center justify-center gap-[2vw] overflow-hidden bg-white p-[2vw]">
          {" "}
          <p
            className="flex items-center justify-center gap-3 text-2xl font-medium tracking-tight text-zinc-950">
            <Bracket className="h-12 text-zinc-950" />
