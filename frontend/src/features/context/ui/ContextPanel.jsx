...
export function ContextPanel({ projectId, ideaId }) {
  const { isGenerating, generatedFiles, error, triggerContextGeneration } = useContextFlow(ideaId)
  ...
  return (
    <div className="bg-surface border border-hairline rounded-xl p-6 shadow-sm">
      ...
      {isGenerating ? (
        <div className="flex items-center gap-3 text-ink-muted text-body-sm" aria-live="polite">
      ...
      {error && (
        <p className="text-destructive text-caption mt-3" aria-live="assertive">{error}</p>
      )}
        ...
        <div className="mt-6 border-t border-hairline pt-6">
          ...
          <div className="space-y-2">
            {generatedFiles.map((file) => (
              <div key={file.name || file} className="flex items-center gap-2 text-ink-muted hover:text-ink transition-colors text-body-sm">
