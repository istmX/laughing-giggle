...
import { getRecentProjects, updateProject, deleteProject } from '@/features/project/api/projects.api'
import toast from 'react-hot-toast'
import { Link } from 'react-router-dom'
import { Modal } from '@/components/ui/Modal'
import { 
  Search, Grid, List as ListIcon, Clock, Star, Pencil, 
...
const GROUP_ORDER = { 'Today': 1, 'Yesterday': 2, 'This Week': 3, 'This Month': 4, 'Older': 5 }
...
export function RecentProjects() {
  ...
  useEffect(() => {
    const fetchProjects = async () => {
      try {
        const res = await getRecentProjects(token)
  ...
  const groupedProjects = useMemo(() => {
    ...
    filteredProjects.forEach(p => {
      const group = getTimeGroup(p.last_opened_at || p.createdAt)
  ...
  return (
    <div className="flex-1 flex flex-col h-full bg-background relative overflow-hidden">
      ...
      {/* Header & Toolbar */}
      <div className="shrink-0 px-6 py-8 md:px-10 lg:px-12 border-b border-hairline bg-canvas z-20">
          ...
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            ...
            <div className="relative w-full flex-1 group">
              ...
              <input
                ...
                placeholder="Search projects... (Press ⌘K)"
                className="w-full min-w-0 bg-surface-soft border border-hairline rounded-lg py-2.5 pl-10 pr-14 text-[14px] text-ink placeholder:text-ink-muted/70 focus:outline-none focus:bg-canvas focus:border-ink/20 focus:ring-4 focus:ring-ink/5 transition-all shadow-sm"
              />
            ...
            <div className="flex items-center gap-3">
              <div className="flex items-center bg-surface-soft border border-hairline rounded-lg p-1">
                <button 
                  onClick={() => setLayout('grid')}
                  className={`p-1.5 rounded-md transition-colors ${layout === 'grid' ? 'bg-canvas shadow-sm text-ink' : 'text-ink-muted hover:text-ink'}`}
                >
                  <Grid className="w-4 h-4" />
                </button>
                <button 
                  onClick={() => setLayout('list')}
                  className={`p-1.5 rounded-md transition-colors ${layout === 'list' ? 'bg-canvas shadow-sm text-ink' : 'text-ink-muted hover:text-ink'}`}
                >
      ...
      {selectedIds.size > 0 && (
        <div className="absolute top-4 left-1/2 -translate-x-1/2 bg-ink text-canvas rounded-full px-6 py-3 flex items-center gap-4 z-50 shadow-lg animate-in slide-in-from-top-4 fade-in duration-200">
          ...
          <button onClick={() => setBulkDeleteConfirm(true)} className="flex items-center gap-1.5 text-[13px] font-[540] hover:text-red-400 transition-colors">
      ...
      {/* Main Content */}
      <div className="flex-1 overflow-y-auto p-6 md:p-10 lg:p-12 [&::-webkit-scrollbar]:hidden [-ms-overflow-style:none] [scrollbar-width:none]">
        <div className="w-full max-w-[1400px] mx-auto pb-24">
          
          {projects.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-32 text-center">
              <div className="w-16 h-16 bg-surface-soft border border-hairline rounded-full flex items-center justify-center mb-6">
            ...
            <div className="space-y-12">
              ...
              {groupedProjects.map(([group, groupProjects]) => (
                <div key={group}>
                  {sortBy === 'opened' && (
                    <h2 className="text-[12px] font-mono tracking-[0.1em] uppercase text-ink-muted/70 mb-4 px-1">
                      {group}
                    </h2>
                  )}
                  
                  <div className={layout === 'grid' ? "grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" : "flex flex-col gap-2"}>
                    {groupProjects.map(project => {
                      ...
                      if (layout === 'grid') {
                        return (
                          <div key={project._id} className={`group relative bg-canvas border rounded-2xl p-5 hover:shadow-md transition-all ${isSelected ? 'border-brand-indigo ring-1 ring-brand-indigo bg-brand-indigo/5' : 'border-hairline hover:border-ink/20'}`}>
                            ...
                            <div className="relative z-10 flex items-start justify-between mb-4">
                              ...
                              <div className="flex items-center gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity">
                                <button onClick={(e) => toggleFavorite(e, project)} className={`p-1.5 rounded-md hover:bg-surface-soft transition-colors ${project.is_favorite ? 'text-warning' : 'text-ink-muted hover:text-ink'}`}>
                                  <Star className="w-4 h-4" fill={project.is_favorite ? 'currentColor' : 'none'} />
                                </button>
                                <button onClick={(e) => handleEditInit(e, project)} className="p-1.5 rounded-md hover:bg-surface-soft text-ink-muted hover:text-ink transition-colors">
                      ...
                      return (
                        <div key={project._id} className={`group relative flex flex-col md:flex-row md:items-center gap-4 bg-canvas border rounded-xl p-4 hover:bg-surface-soft transition-colors ${isSelected ? 'border-brand-indigo bg-brand-indigo/5' : 'border-transparent hover:border-hairline'}`}>
                          ...
                          <div className="relative z-10 flex items-center justify-end w-24 gap-1 opacity-100 md:opacity-0 group-hover:opacity-100 transition-opacity self-end md:self-auto mt-2 md:mt-0">
                            <button onClick={(e) => toggleFavorite(e, project)} className={`p-1.5 rounded-md hover:bg-surface-elevated transition-colors ${project.is_favorite ? 'text-warning' : 'text-ink-muted hover:text-ink'}`}>
                              <Star className="w-4 h-4" fill={project.is_favorite ? 'currentColor' : 'none'} />
                            </button>
      ...
      <Modal isOpen={bulkDeleteConfirm} onClose={() => setBulkDeleteConfirm(false)} title="Delete Projects">
        ...
        <div className="flex items-center justify-end gap-3">
          ...
          <button onClick={handleBulkDelete} className="px-5 py-2.5 text-[14px] font-[540] text-canvas bg-destructive hover:bg-destructive/90 rounded-full focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-destructive/30 transition-all shadow-sm">Delete Projects</button>
        </div>
      </Modal>
