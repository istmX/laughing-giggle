...
// Helper for badge rarity/colors
...
export const PublicProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        // Naive initial check if the logged in user is in the follower array 
        // We'll need the backend to return followers list or we just assume false until fixed
  ...
  if (activeTab === 'popular') {
    ...
    // featured: mock logic, maybe top 2
  ...
  return (
    <div className="min-h-screen bg-background pb-24">
      ...
      {/* Hero Banner Area */}
      <div className="h-48 md:h-64 bg-gradient-to-br from-surface-soft via-background to-surface-soft border-b border-hairline relative overflow-hidden">
        <div className="absolute inset-0 bg-[radial-gradient(circle_at_50%_120%,rgba(0,0,0,0.03),transparent)]" />
      </div>

      <div className="max-w-6xl mx-auto px-6 md:px-10 -mt-20 md:-mt-24 relative z-10 space-y-12">
        ...
        >
          <div className="flex flex-col md:flex-row gap-6 md:gap-8 items-start md:items-end">
            <div className="w-32 h-32 md:w-40 md:h-40 rounded-2xl bg-canvas border-4 border-background shadow-sm overflow-hidden shrink-0">
            ...
            <div className="pb-2">
              <h1 className="text-display-lg font-540 tracking-display-lg text-ink flex items-center gap-3 mb-1">
                {user.name}
                {user.isVerified && <BadgeCheck className="w-7 h-7 text-blue-500 fill-blue-500/10" strokeWidth={1.5} />}
              </h1>
          ...
          <div className="flex items-center gap-3 w-full md:w-auto pb-2">
            {currentUser?.username !== user.username && (
              <button 
                ...
                className={`flex-1 md:flex-none text-button font-480 px-6 py-2.5 rounded-full hover:opacity-90 transition-opacity flex items-center justify-center gap-2 ${isFollowing ? 'bg-surface-soft border border-hairline text-ink' : 'bg-ink text-background'}`}
              >
        ...
        >
          <div className="lg:col-span-2 space-y-6">
            {user.bio ? (
              <p className="text-body-lg font-320 text-ink/90 leading-relaxed max-w-3xl">
                {user.bio}
              </p>
            ) : (
              <p className="text-body-lg font-320 text-ink-muted italic max-w-3xl">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  {user.loyaltyBadges.map((badge, idx) => {
                    ...
                    return (
                      <div 
                        key={idx} 
                        className={`flex items-center gap-2 px-3 py-1.5 rounded-full border text-caption font-500 tracking-wide uppercase transition-transform hover:scale-105 cursor-default ${style}`}
        ...
        >
            ...
            <div className="flex p-1 bg-surface-soft border border-hairline rounded-full inline-flex w-max">
              {['featured', 'newest', 'popular'].map(tab => (
                <button
                  ...
                  className={`px-5 py-1.5 rounded-full text-caption font-480 capitalize transition-all ${activeTab === tab ? 'bg-canvas text-ink shadow-sm' : 'text-ink-muted hover:text-ink'}`}
                >
                  {tab}
                </button>
              ))}
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {displayTemplates.length === 0 ? (
              <div className="col-span-1 md:col-span-2 bg-canvas border border-dashed border-hairline rounded-3xl p-16 flex flex-col items-center justify-center text-center w-full">
                ...
                <p className="text-body font-320 text-ink-muted max-w-[400px] w-full mx-auto">
                        ...
                        <html>
                          <head>
                            <style>
                              ...
                              .card { background: white; padding: 2.5rem; border-radius: 16px; box-shadow: 0 8px 24px rgba(0,0,0,0.04); text-align: center; border: 1px solid #eaeaea; }
                              h1 { margin-top: 0; font-size: 1.75rem; font-weight: 600; color: #111; }
                              button { background: #111; color: white; border: none; padding: 0.75rem 1.5rem; border-radius: 99px; font-weight: 500; cursor: pointer; margin-top: 1rem; }
                            </style>
                          </head>
                          <body>
                            <div class="card">
                              <h1>${template.name}</h1>
                              <p style="color: #666; font-size: 15px;">Preview rendering active...</p>
